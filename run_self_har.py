...
# ===============================================================
#  PATCH 2 — Add Transformer support to SelfHAR pipeline
#  Adds: config["core_model"] = ["cnn", "transformer"]
# ===============================================================

from self_har_models import (
    create_transformer_core_model,
    build_teacher_student_transformer_models,
)

def build_core_model_from_config(input_shape, config):
    """
    Selects CNN or Transformer based on config["core_model"].
    
    Options:
        "cnn" (default) → create_1d_conv_core_model
        "transformer"   → create_transformer_core_model
    """

    model_type = config.get("core_model", "cnn").lower()

    if model_type == "transformer":
        print("⚡ Using Transformer Core Model")
        return create_transformer_core_model(input_shape)

    else:
        print("🧱 Using CNN Core Model (default)")
        # NOTE: This function already exists in the original code
        return create_1d_conv_core_model(input_shape)



def build_teacher_model_from_config(input_shape, num_classes, config):
    """
    Select Teacher CNN or Teacher Transformer.
    """
    model_type = config.get("core_model", "cnn").lower()

    if model_type == "transformer":
        teacher, _ = build_teacher_student_transformer_models(
            input_shape=input_shape,
            num_classes=num_classes,
        )
        return teacher

    print("🧱 Using CNN Teacher model")
    return build_teacher_model(input_shape, num_classes)



def build_student_model_from_config(input_shape, num_classes, config):
    """
    Select Student CNN or Student Transformer.
    """
    model_type = config.get("core_model", "cnn").lower()

    if model_type == "transformer":
        _, student = build_teacher_student_transformer_models(
            input_shape=input_shape,
            num_classes=num_classes,
        )
        return student

    print("🧱 Using CNN Student model")
    return build_student_model(input_shape, num_classes)




# ===================================================================
# PATCH 2 — Pipeline Transformer Activation
# ===================================================================
...
def build_teacher_model_from_config(input_shape, num_classes, config):
    ...
    if model_type == "transformer":
        ...
        print("⚡ Using Teacher Transformer")
        return teacher

    print("🧱 Using Teacher CNN")
    return build_teacher_model(input_shape, num_classes)
...
def build_student_model_from_config(input_shape, num_classes, config):
    ...
    if model_type == "transformer":
        ...
        print("⚡ Using Student Transformer")
        return student

    print("🧱 Using Student CNN")
    return build_student_model(input_shape, num_classes)
